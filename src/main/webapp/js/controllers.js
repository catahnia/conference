...
conferenceApp.controllers.controller('ConferenceDetailCtrl', function ($scope, $log, $routeParams, HTTP_ERRORS) {
    ...
    $scope.done = false;  // {flag} loading the map
        ...
        gapi.client.conference.getConference({
            ...
            $scope.$apply(function () {
                ...
                if (resp.error) {
                    ...
                    $scope.done = true;  // {flag} loading the map
                }
            });
        });
...
}).directive('myMaps', function($rootScope) {
 ...
 * @name CityCtrl
 * @description
 * A controller used for the autocomplete the location of the Conference.
 */

conferenceApp.controllers.controller('CityCtrl', function ($scope) {
    //$scope = {'conference.city': '', 'fromLat': '', 'fromLng' : ''};
    var inputFrom = document.getElementById('address_location');
    var autocompleteFrom = new google.maps.places.Autocomplete(inputFrom);
    google.maps.event.addListener(autocompleteFrom, 'place_changed', function() {
        var place = autocompleteFrom.getPlace();
        $scope.conference.lat = place.geometry.location.lat();
        $scope.conference.lng = place.geometry.location.lng();
        $scope.conference.city = place.formatted_address;
        $scope.$apply();
    });
});
...
conferenceApp.controllers.controller('CreateSessionCtrl',
    function ($scope, $log, oauth2Provider, HTTP_ERRORS) {
            ...
            execute(function (resp) {
                $scope.$apply(function () {
                    ...
                    if (resp.error) {
                        ...
                        // The request has succeeded.
                        $scope.submitted = false;
                        $scope.messages = 'Query succeeded : Showing all conferences';
                        $scope.alertStatus = 'success';
                        $log.info($scope.messages);
                        $scope.conferences = [];
                        angular.forEach(resp.items, function (conference) {
        ...
        $scope.getSelection = function () {
            $scope.key = $scope.selection.websafeKey;
        }
         ...
         */
         $scope.isValidDates = function () {
             var sessionStartDate = new Date($scope.session.startDate).getTime();
             var selectionStartDate = new Date($scope.selection.startDate).getTime();
             var sessionStartTime = new Date($scope.session.startTime).getTime();
             var selectionEndDate = new Date($scope.selection.endDate).getTime();

             if (sessionStartDate >= selectionStartDate && sessionStartDate <= selectionEndDate
                 && sessionStartTime >= selectionStartDate && sessionStartTime <= selectionEndDate) {
                 return true;
        ...
        $scope.createSession = function (sessionForm) {
            ...
            gapi.client.conference.createSession({
                webSafeKey: $scope.key}, $scope.session).
            execute(function (resp) {
                $scope.$apply(function () {
                    $scope.loading = false;
                    if (resp.error) {
